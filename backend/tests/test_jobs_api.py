...
def test_get_job_includes_application_fields_after_mark_applied(client):
    test_client, SessionLocal = client
    db = SessionLocal()
    job = crud.create_or_update_job(
        db,
        schemas.JobCreate(
            ...
            source="simplify",
            source_url="https://example.com/acme-backend",
            discovered_date=date(2026, 8, 19),
        ),
    )
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["applied_date"] == "2026-08-20"
    assert body["application_number"] == "APP-789"
    assert body["notes"] == "Applied via referral"

    get_response = test_client.get(f"/jobs/{job.id}")
    assert get_response.status_code == 200
    get_body = get_response.json()
    assert get_body["applied_date"] == "2026-08-20"
    assert get_body["application_number"] == "APP-789"
    assert get_body["notes"] == "Applied via referral"


def test_applied_to_rejected_transition_via_api(client):
    test_client, SessionLocal = client
    db = SessionLocal()
    job = crud.create_or_update_job(
        db,
        schemas.JobCreate(
            company="Acme",
            role_title="Platform Intern",
            source="simplify",
            source_url="https://example.com/acme-platform",
            discovered_date=date(2026, 8, 19),
        ),
    )
    crud.update_job_status(db, job, JobStatus.tailored)
    ...
    response = test_client.patch(f"/jobs/{job.id}", json={"status": "rejected"})
    assert response.status_code == 200
